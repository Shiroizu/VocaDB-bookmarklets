javascript: (async function () {
  ...
  const checkAlbumInDatabase = async (url) => {
    console.log(
      `Checking if ${url} is added as an album crossfade`
    );
    const apiUrl = `${CONFIG.baseUrl}/api/albums?query=${url}`;
  ...
  const addButtonsForNicoVideo = async videoId => {
    ...
    const albumData = await checkAlbumInDatabase(nicoUrl);

    if (albumData === null) {
      // Video not in database - add "Add" and "Info" buttons
      console.log("Album not in database");
      const addBtn = createButton(
        "Add",
        CONFIG.colors.add,
        `${CONFIG.baseUrl}/Album/Create`
      );
  ...
  const handleDirectVideoPage = async () => {
    ...
    try {
      ...
      const albumData = await checkAlbumInDatabase(videoUrl);

      if (albumData) {
        window.location.assign(`${CONFIG.baseUrl}/Al/${albumData.id}`);
      } else {
        window.open(`${CONFIG.baseUrl}/Album/Create`);
      }
    } catch (error) {
      alert(error.message);
    }
